...
/**
 * Code added by MAL to relocate vector tables
 */
#if defined(MAL_RELOCATE_VECTOR_TABLE)
#include "stm32f0/stm32f0xx.h"
#include "cm0/core_cm0.h"
#include "stm32f0/stm32f0xx_rcc.h"
#include "stm32f0/stm32f0xx_syscfg.h"
__IO uint32_t VectorTable[49] __attribute__((section(".RAMVectorTable")));
#endif
...
void mal_stm32f0_relocate_vector_table(void) {
    ...
    uint32_t i = 0;
    for(i = 0; i < 48; i++) {
      uint32_t vectors_start = (uint32_t)g_pfnVectors;
      VectorTable[i] = *(__IO uint32_t*)(vectors_start + (i<<2));
    }
