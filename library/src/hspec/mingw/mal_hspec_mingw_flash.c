...
void mal_hspec_mingw_flash_set_flash(mal_hspec_mingw_flash_info_s *info) {
	...
	for (uint32_t i = 0; i < flash_info.page_count; i++) {
		...
		// Erase page
		mal_hspec_mingw_flash_erase_page(i);
	}
}
...
uint32_t mal_hspec_mingw_flash_get_page_size(uint32_t page) {
	if (page >= mal_hspec_mingw_flash_get_page_count()) {
		return 0;
	}
...
mal_error_e mal_hspec_mingw_flash_write_uint8_values(uint64_t start_address, uint8_t *values, uint32_t count) {
	for (uint32_t i = 0; i < count; i++) {
		...
		uint32_t current_page = mal_hspec_mingw_flash_get_page_from_address(current_address);
		uint64_t address_offset = current_address - mal_hspec_mingw_flash_get_page_start_address(current_page);
		// Write byte
...
uint32_t mal_hspec_mingw_flash_get_page_from_address(uint64_t address) {
	...
	for (uint32_t i = 0; i < flash_info.page_count; i++) {
		...
		uint64_t current_end_address = current_start_address + mal_hspec_mingw_flash_get_page_size(i) - 1;
		...
		current_start_address += mal_hspec_mingw_flash_get_page_size(i);
	}
	return 0;
}
...
uint8_t mal_hspec_mingw_flash_read_uint8(uint64_t address) {
	uint32_t page = mal_hspec_mingw_flash_get_page_from_address(address);
	uint64_t address_offset = address - mal_hspec_mingw_flash_get_page_start_address(page);
	return flash_pages[page][address_offset];
}

int8_t mal_hspec_mingw_flash_read_int8(uint64_t address) {
	return (int8_t)mal_hspec_mingw_flash_read_uint8(address);
}

uint16_t mal_hspec_mingw_flash_read_uint16(uint64_t address) {
	uint8_t byte_0 = mal_hspec_mingw_flash_read_uint8(address);
	uint8_t byte_1 = mal_hspec_mingw_flash_read_uint8(address + 1);
...
int16_t	mal_hspec_mingw_flash_read_int16(uint64_t address) {
	return (int16_t)mal_hspec_mingw_flash_read_uint16(address);
}

uint32_t mal_hspec_mingw_flash_read_uint32(uint64_t address) {
	uint16_t word_0 = mal_hspec_mingw_flash_read_uint16(address);
	uint16_t word_1 = mal_hspec_mingw_flash_read_uint16(address + 2);
...
int32_t	mal_hspec_mingw_flash_read_int32(uint64_t address) {
	return (int32_t)mal_hspec_mingw_flash_read_uint32(address);
}

uint64_t mal_hspec_mingw_flash_read_uint64(uint64_t address) {
	uint32_t word_0 = mal_hspec_mingw_flash_read_uint32(address);
	uint32_t word_1 = mal_hspec_mingw_flash_read_uint32(address + 4);
...
int64_t	mal_hspec_mingw_flash_read_int64(uint64_t address) {
	return (int64_t)mal_hspec_mingw_flash_read_uint64(address);
}
